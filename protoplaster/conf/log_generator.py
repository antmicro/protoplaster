...
class LogGenerator:
    def __init__(self, log_path):
        ...
        self.retrieve_field = {
            "device_name": (
                lambda item, report: dict(item.user_properties).get("device", "unknown")
            ),
            "test_name": (lambda item, report: item.name.removeprefix("test_")),
            "module": (
            ...
            "user_properties": (
                lambda item, report: {
                    prop[0]: prop[1] for prop in report.user_properties[1:]
                }
            ),
        }
